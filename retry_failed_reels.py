...
# Retry specific Reel indices (1, 3, 7)
RETRY_INDICES = [1, 3, 7]
...
def run():
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write("=== RETRY FABULOUS REELS PUBLISHER ===\n")

    token = open(TOKEN_FILE, "r", encoding="utf-8").read().strip()
    with open(CSV_FILE, "r", encoding="utf-8") as f:
        items = list(csv.DictReader(f))

    log(f"Retrying publication for Reels: {RETRY_INDICES}...")
    ...
    for idx in RETRY_INDICES:
        ...
        log(f"\n[RETRY {idx}/40] Publishing '{title}' ({content_type})")
